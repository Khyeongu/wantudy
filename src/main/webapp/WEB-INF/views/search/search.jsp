							...
							<div class="hero__search__form">
								<form>
									<select id="searchCondition"
										class="hero__search__categories arrow_carrot-down">
										<option value="name"
											<c:if test="${searchDTO.searchCondition == 'name'}"> selected </c:if>>제목</option>
										<option value="content"
											<c:if test="${searchDTO.searchCondition == 'content'}"> selected </c:if>>내용</option>
										<option value="categoty"
											<c:if test="${searchDTO.searchCondition == 'category'}"> selected </c:if>>카테고리</option>
									</select> <input type="text" id="searchKeyword" required="required"
				...
				<div class="col-lg-10 col-md-7">
					...
					<div class="row">
						...
						<c:forEach items="${recentStudyList}" var="rsl">
							...
							<!-- 카드 시작 -->
							<div class="col-lg-4" data-toggle="modal"
								data-target="#exampleModal${rsl.no}" style="cursor: pointer;">
								<!-- Modal -->
								<div class="modal fade" id="exampleModal${rsl.no}" tabindex="-1"
										...
										<div class="modal-content">
												...
												<div class="row">
													<div class="col-lg-12">
														<div class="studyinfo" id="studyinfo">
															<form id="studyInfoForm" name="studyInfoForm"
																method="post">
																<div>
																	<h4>스터디 이름</h4>
																	<input type="text" id="name" name="name"
																		value="${rsl.name}" disabled="disabled">
																</div>
																<div>
																	<h4>스터디 내용 //글상자 크기 변경해야함</h4>
																	<input type="text" id="content" name="content"
																		value="${rsl.content}" disabled="disabled">
																</div>
																<div>
																	<h4>시작 날짜</h4>
																	<input autocomplete="off" class="form-control"
																		id="startdate" name="startdate" type="text"
																		value="${rsl.startdate}" placeholder="시작 날짜"
																		disabled="disabled" />
																</div>
																<div>
																	<h4>종료 날짜</h4>
																	<input autocomplete="off" class="form-control"
																		...
																		value="${rsl.enddate}" placeholder="종료 날짜"
																		disabled="disabled" />
																</div>
																<div>
																	<h4>최대 인원</h4>
																	<input type="text" id="capacity" name="capacity"
																		value="${rsl.capacity}" disabled="disabled">
																</div>
																<div>
																	<h4>카테고리</h4>
																</div>
																<div>
																	<input type="text" id="capacity" name="capacity"
																		value="${rsl.category}" disabled="disabled">
																</div>
															</form>
														</div>
													</div>
											...
											<div class="modal-footer">
												<button type="button" class="btn btn-primary"
													onclick="enroll(${rsl.no})">신청하기</button>
